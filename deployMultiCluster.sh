...
YAMLs=($(ls ./k8s/*-deployment-*.yaml))


for YAML in "${YAMLs[@]}"; do

    KUBE_SERVER=$(echo $YAML | sed -e "s/.yaml//" -e "s/.\/k8s\///" | awk -F "-" '{print $1}')
    KUBE_NAMESPACE=$(grep 'namespace' $YAML | sed 's/namespace://')
    DOCKER_ORG=$(grep 'image:' $YAML | awk -F "/" '{ print $2 }')
    DOCKER_REPO=$(grep 'image:' $YAML | awk -F "/" '{ print $3 }')


    # ////


    # TODO what about KUBE_CA, KUBE_CONTAINERS, KUBE_DEPLOYMENTS
